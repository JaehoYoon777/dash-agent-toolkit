...
## Route-smoke navigation must SETTLE between gotos

A multi-route smoke that does `page.goto(A); wait_for_selector; page.goto(B); ...` in a tight loop will cancel A's in-flight callback XHRs the instant it navigates to B. dash-renderer logs the cancelled request as `Callback failed: the server did not respond.` on the console — a benign artifact that the console-error net (correctly) flags as a failure. It is NOT a server bug and NOT masked by a threaded dev server; the CLIENT cancels on navigation. It surfaces once a page grows heavy enough that a mount callback is still in flight when the loop advances (observed the day a nav index grew to 33 cards). Fix the TEST, do not silence the message: add `page.wait_for_load_state("networkidle")` after each route's content assertion so the callbacks drain before the next goto. If "server did not respond" still appears after network-idle waits, THEN it is real (a hung/dead server thread).
